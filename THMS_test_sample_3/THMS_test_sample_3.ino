...
#define SCHNEIDER_ID    1    // Schneider Meter Slave ID = 1
#define TPR702_ID       2    // 📌 TPR-702 Slave ID (Set to 2 if both on same RS485 bus, or 1 if separate)
...
// --------------------------------------------------------------------------
// 📌 4-20mA & OLTC TAP POSITION (CALIBRATED: Tap 1 = 0.44V, Tap 17 = 3.13V)
// --------------------------------------------------------------------------
void read4to20mASensor() {
  analog_raw_adc = analogRead(ANALOG_4_20MA_PIN);
  
  // 1. ESP32 Analog Pin se Measured Voltage (0.0V to 3.3V)
  float voltage = (analog_raw_adc / 4095.0) * 3.3;

  // 2. Wire Cut / Open Circuit / Fault Check (<0.25V ya >3.35V)
  if (voltage < 0.25 || voltage > 3.35) {
    oltcData.sensor_ok = false;
    oltcData.currentTap = 0;
    analog_4_20mA_val = 0.0;
    return;
  }
  oltcData.sensor_ok = true;

  // 3. Exact 17-Tap Calibration Formula
  // Base = 0.44V (Tap 1), Max = 3.13V (Tap 17), Gap per Tap = 0.168125V
  int calculatedTap = round(1.0 + ((voltage - 0.44) / 0.168125));
  if (calculatedTap < 1)  calculatedTap = 1;
  if (calculatedTap > 17) calculatedTap = 17;
  oltcData.currentTap = calculatedTap;

  // 4. Equivalent 4-20mA Current Value Calculate karein (Display & MQTT ke liye)
  analog_4_20mA_val = 4.0 + ((voltage - 0.44) / 2.69) * 16.0;

  // 5. First Bootup State Setup
  if (lastConfirmedTap == -1) {
    lastConfirmedTap = calculatedTap;
    return;
  }

  // 6. Tap Change Transition (2-Second Anti-Glitch Debounce & Flash Counter +1)
  if (calculatedTap != lastConfirmedTap) {
    if (calculatedTap != candidateNewTap) {
      candidateNewTap = calculatedTap;
      tapDebounceTimer = millis();
    } else if (millis() - tapDebounceTimer > 2000) {
      int stepsMoved = abs(candidateNewTap - lastConfirmedTap); // e.g. 17 se 10 = 7 steps
      
      oltcData.tapCounter += stepsMoved; // Pure 7 steps add honge
      nvsStorage.putUInt("tap_count", oltcData.tapCounter); // Flash me save
      Serial.printf("\n⚡ [OLTC EVENT] Tap: %d ➔ %d | Steps: +%d | Total Ops: %u\n\n", 
                    lastConfirmedTap, candidateNewTap, stepsMoved, oltcData.tapCounter);
      lastConfirmedTap = candidateNewTap;
    }
  } else {
    candidateNewTap = -1;
  }
...
// --------------------------------------------------------------------------
// READ SCHNEIDER METER DATA (66 REGISTERS)
// --------------------------------------------------------------------------
bool readSchneiderMeterData() {
  uint16_t buffer[120];

  bool s1_ok = readHoldingRegistersSchneider(SCHNEIDER_ID, 2700, 24, buffer);
  if (s1_ok) {
  ...
  bool s2_ok = readHoldingRegistersSchneider(SCHNEIDER_ID, 3000, 112, buffer);
  if (s2_ok) {
  ...
  bool s3_ok = readHoldingRegistersSchneider(SCHNEIDER_ID, 21300, 36, buffer);
  if (s3_ok) {
...
void printAllDataToSerial() {
  ...
  logPrintln("\n" + getFormattedTime() + " =================================================================");
  logPrint("   FULL TELEMETRY SCAN (SCHNEIDER SLAVE: "); logPrint(String(SCHNEIDER_ID));
  logPrint(" | TPR-702 SLAVE: "); logPrint(String(TPR702_ID));
  logPrintln(is_meter_data_available ? " | METER: ONLINE ✅)" : " | METER: OFFLINE ❌)");
  logPrintln("=================================================================");
  ...
  logPrintln("\n🟢 [SECTION 5] 📌 TPR-702 TRANSFORMER TEMPERATURE CONTROLLER:");
  if (tprData.is_valid) {
    logPrint("  69. Oil Temperature (Reg 40001):          "); logPrint(String(tprData.oilTemp)); logPrintln(" °C");
    logPrint("  70. HV Winding-1 Temp (Reg 40002):        "); logPrint(String(tprData.hvTemp)); logPrintln(" °C");
    logPrint("  71. LV Winding-2 Temp (Reg 40003):        "); logPrint(String(tprData.lvTemp)); logPrintln(" °C");
  } else {
    logPrintln("  ❌ TPR-702 Modbus Read Failed!");
  }
  logPrintln("=================================================================\n");
...
void loop() {
  ...
  // 📌 1. READ SCHNEIDER EM6436H ENERGY METER
  readSchneiderMeterData();

  delay(50); // Short RS485 bus gap between meters

  // 📌 2. READ TPR-702 TRANSFORMER TEMPERATURE CONTROLLER
  tprData.is_valid = readTPR702(TPR702_ID, tprData.oilTemp, tprData.hvTemp, tprData.lvTemp);

  // 📌 3. READ 4-20mA SENSOR
  read4to20mASensor();

  // 📌 4. PRINT FULL SCAN TO SERIAL & LOG TO SD CARD
  ...
  if (sd_card_mounted) {
    String logLine = "kWh:" + String(sec1.totalActive_kWh, 2) + 
                     ...
                     " | OilTemp:" + String(tprData.oilTemp) + "C" +
                     " | HVTemp:" + String(tprData.hvTemp) + "C" +
                     " | LVTemp:" + String(tprData.lvTemp) + "C";
    logDataToSD(logLine);
  }
